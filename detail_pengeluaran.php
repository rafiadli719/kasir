...
try {
    if ($jenis_data === 'semua' || $jenis_data === 'kasir') {
        $sql_cabang_kasir = "SELECT DISTINCT nama_cabang FROM view_pengeluaran_kasir WHERE nama_cabang IS NOT NULL";
    ...
    if (($jenis_data === 'semua' || $jenis_data === 'pusat') && $is_super_admin) {
        $sql_cabang_pusat = "SELECT DISTINCT nama_cabang FROM view_pengeluaran_pusat WHERE nama_cabang IS NOT NULL";
    ...
    if (($jenis_data === 'semua' || $jenis_data === 'pusat') && $is_super_admin) {
        $sql_cabang_pusat = "SELECT DISTINCT cabang as nama_cabang FROM pengeluaran_pusat WHERE cabang IS NOT NULL";
        $stmt_cabang_pusat = $pdo->query($sql_cabang_pusat);
        $cabang_pusat = $stmt_cabang_pusat->fetchAll(PDO::FETCH_ASSOC);
        $cabang_list = array_merge($cabang_list, $cabang_pusat);
    }
...
$total_records = count($pengeluaran);
...
<head>
    ...
    <title>Detail Pengeluaran - Admin Dashboard</title>
...
<div class="main-content">
    ...
    <div class="welcome-card">
        <h1><i class="fas fa-file-invoice-dollar"></i> Detail Pengeluaran Terpadu</h1>
        <p style="color: var(--text-muted); margin-bottom: 0;">Monitor dan analisis data pengeluaran dari kasir dan pusat dengan sorting dinamis dan performa yang dioptimasi</p>
        <div class="info-tags">
    ...
    <div class="filter-card">
        ...
        <div style="margin-bottom: 20px;">
            <a href="?jenis_data=semua&<?php echo http_build_query(array_filter(['tanggal_awal' => $tanggal_awal, 'tanggal_akhir' => $tanggal_akhir, 'kategori' => $kategori, 'cabang' => $cabang, 'sort_by' => $sort_by, 'sort_order' => $sort_order])); ?>"
               class="btn btn-warning btn-tab <?php echo $jenis_data === 'semua' ? 'active' : ''; ?>">
                <i class="fas fa-layer-group"></i> Semua Data (Kasir + Pusat)
            </a>
            <a href="?jenis_data=kasir&<?php echo http_build_query(array_filter(['tanggal_awal' => $tanggal_awal, 'tanggal_akhir' => $tanggal_akhir, 'kategori' => $kategori, 'cabang' => $cabang, 'sort_by' => $sort_by, 'sort_order' => $sort_order])); ?>"
               class="btn btn-info btn-tab <?php echo $jenis_data === 'kasir' ? 'active' : ''; ?>">
                <i class="fas fa-cash-register"></i> Pengeluaran Kasir
            </a>
            <?php if ($is_super_admin): ?>
                <a href="?jenis_data=pusat&<?php echo http_build_query(array_filter(['tanggal_awal' => $tanggal_awal, 'tanggal_akhir' => $tanggal_akhir, 'kategori' => $kategori, 'cabang' => $cabang, 'sort_by' => $sort_by, 'sort_order' => $sort_order])); ?>"
                   class="btn btn-primary btn-tab <?php echo $jenis_data === 'pusat' ? 'active' : ''; ?>">
                    <i class="fas fa-building"></i> Pengeluaran Pusat
                </a>
            <?php endif; ?>
        </div>
    </div>
    ...
    <!-- Filter Card -->
    <div class="filter-card">
        <h3><i class="fas fa-filter"></i> Filter Data Pengeluaran <?php echo ucfirst($jenis_data); ?></h3>
        <form method="GET" action="">
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <select name="kategori" id="kategori" class="form-control">
                        ...
                        <option value="biaya" <?php echo isset($_GET['kategori']) && $_GET['kategori'] == 'biaya' ? 'selected' : ''; ?>>Biaya</option>
                        <option value="non_biaya" <?php echo isset($_GET['kategori']) && $_GET['kategori'] == 'non_biaya' ? 'selected' : ''; ?>>Non-Biaya</option>
                    </select>
                </div>
                <div class="form-group">
            ...
            <div class="filter-actions">
                ...
                <a href="detail_pengeluaran.php?jenis_data=<?php echo $jenis_data; ?>" class="btn btn-secondary">
    ...
    <?php if (count($pengeluaran) > 0): ?>
        <div class="table-container">
            <div class="table-header">
                <h3><i class="fas fa-table"></i> Data Pengeluaran <?php echo ucfirst($jenis_data); ?></h3>
                <div style="display: flex; gap: 15px; align-items: center;">
                    ...
                    <div style="font-size: 14px; color: var(--text-muted);">
                        Menampilkan <?php echo number_format(count($pengeluaran)); ?> dari total data
                    </div>
                </div>
            </div>
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th class="sortable <?php echo ($sort_by === 'umur_pakai') ? 'active' : ''; ?>" 
                                onclick="window.location.href='<?php echo getSortUrl('umur_pakai', $sort_by, $sort_order); ?>'">
                                Umur Pakai (Bulan)
                                <span class="sort-icon"><?php echo getSortIcon('umur_pakai', $sort_by, $sort_order); ?></span>
                            </th>
                    ...
                    <tbody>
                        <?php foreach ($pengeluaran as $index => $data): ?>
                            <tr>
                                ...
                                <td><code><?php echo htmlspecialchars($data['kode_transaksi'], ENT_QUOTES); ?></code></td>
                                ...
                                <td>
                                    <span class="kategori-badge kategori-<?php echo htmlspecialchars($data['kategori_akun'], ENT_QUOTES); ?>">
                                        <?php echo htmlspecialchars(ucfirst(str_replace('_', ' ', $data['kategori_akun'])), ENT_QUOTES); ?>
                                    </span>
                                </td>
                                ...
                                <td>
                                    <?php 
                                    if ($data['umur_pakai']) {
                                        echo htmlspecialchars($data['umur_pakai'], ENT_QUOTES);
                                    } else {
    ...
    <?php else: ?>
        <div class="table-container">
            ...
            <div class="no-data">
                ...
                <strong>Tidak ada data pengeluaran</strong><br>
    ...
    <?php if (count($pengeluaran) > 0): ?>
        <div class="alert alert-info">
            ...
            <?php if ($jenis_data === 'semua'): ?>
                Menampilkan data gabungan dari pengeluaran kasir dan pusat.
            <?php endif; ?>
        </div>
    <?php endif; ?>
