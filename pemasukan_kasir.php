...
require_once 'process_closing_transaction.php'; // Include closing transaction functions

// Set timezone ke zona waktu lokal (misal: Asia/Jakarta)
...
// Include closing transaction processor
require_once 'process_closing_transaction.php';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_pemasukan'])) {
    ...
    // Insert into the `pemasukan_kasir` table
    $query = "INSERT INTO pemasukan_kasir (kode_transaksi, kode_karyawan, kode_akun, jumlah, keterangan_transaksi, tanggal, waktu, nomor_transaksi_closing) 
              VALUES ('$kode_transaksi', '$kode_karyawan', '$kode_akun', '$jumlah', '$keterangan_transaksi', '$tanggal', '$waktu', " . 
              ($nomor_transaksi_closing ? "'$nomor_transaksi_closing'" : "NULL") . ")";
    
    if ($conn->query($query) === TRUE) {
        // Jika transaksi "DARI CLOSING", proses closing transaction
        if (strtoupper($nama_transaksi) === 'DARI CLOSING' && !empty($nomor_transaksi_closing)) {
            // Ambil ID pemasukan yang baru saja diinsert
            $pemasukan_id = $conn->insert_id;
            
            // Update bukti_transaksi di kasir_transactions
            $update_bukti = "UPDATE kasir_transactions SET bukti_transaksi = '$pemasukan_id' WHERE kode_transaksi = '$nomor_transaksi_closing'";
            $conn->query($update_bukti);
        ...
        header("Location: pemasukan_kasir.php?kode_transaksi=$kode_transaksi&success=1");
        exit;
    } else {
...
// Retrieve data from the `master_akun` table where jenis_akun is 'pemasukan'
$sql = "SELECT * FROM master_akun WHERE jenis_akun = 'pemasukan'";
...
if (empty($available_closing)) {
    error_log("No available closing transactions found for cabang: " . $user_nama_cabang);
} else {
    error_log("Found " . count($available_closing) . " available closing transactions for cabang: " . $user_nama_cabang);
}

// Query for displaying latest pemasukan data
$sqlKasMasuk = "
    SELECT pk.*, u.nama_karyawan, u.nama_cabang
            ...
            <form method="POST" action="" id="pemasukanForm">
                ...
                <div class="form-group" id="closing_section" style="display: none;">
                    <label for="nomor_transaksi_closing" class="form-label">
                        ...
                        <span class="form-hint">Pilih transaksi closing dari cabang Anda yang sudah end proses dan belum disetor.</span>
                    </label>
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if (!empty($row['nomor_transaksi_closing'])): ?>
    ...
    <script>
        ...
        function detectClosingType(kodeTransaksi) {
            fetch('detect_closing_type.php', {
                ...
                body: 'kode_transaksi=' + encodeURIComponent(kodeTransaksi) + '&nama_cabang=' + encodeURIComponent('<?php echo $_SESSION['nama_cabang']; ?>')
            })
