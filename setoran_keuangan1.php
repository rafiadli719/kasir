...
function updateSetoranKeuanganStatus($pdo, $kode_setoran, $kode_karyawan, $validation_columns_exist, $selisih_fisik_is_generated) {
    // Count total and validated transactions
    $sql_count_total = "SELECT COUNT(*) FROM kasir_transactions WHERE kode_setoran = ? AND deposit_status IN ('Diterima Staff Keuangan', 'Validasi Keuangan OK', 'Validasi Keuangan SELISIH')";
    $stmt_total = $pdo->prepare($sql_count_total);
    $stmt_total->execute([$kode_setoran]);
    $total_transaksi = $stmt_total->fetchColumn();

    $sql_count_validated = "SELECT COUNT(*) FROM kasir_transactions WHERE kode_setoran = ? AND deposit_status IN ('Validasi Keuangan OK', 'Validasi Keuangan SELISIH')";
    ...
    if ($total_transaksi == $validated_transaksi && $total_transaksi > 0) {
        // Check if there are any selisih
        ...
        $setoran_status = ($selisih_count > 0) ? 'Validasi Keuangan SELISIH' : 'Validasi Keuangan OK';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['setor_bank'])) {
    ...
    if (empty($setoran_ids)) {
        ...
        // Get kode_cabang from selected rekening
        $sql_get_cabang = "SELECT kode_cabang FROM master_rekening_cabang WHERE id = ?";
        $stmt_get_cabang = $pdo->prepare($sql_get_cabang);
        $stmt_get_cabang->execute([$rekening_cabang_id]);
        $target_kode_cabang = $stmt_get_cabang->fetchColumn();
        
        if (!$target_kode_cabang) {
            $error = "Rekening cabang tidak ditemukan.";
        } else {
            // Check if all setoran are from the same cabang and have no selisih
            $sql_check = "SELECT COUNT(*) FROM setoran_keuangan 
                         WHERE id IN (" . implode(',', $placeholders) . ") 
                         AND (status = 'Validasi Keuangan SELISIH' OR kode_cabang != ?)";
            $stmt_check = $pdo->prepare($sql_check);
            $params_check = array_merge($setoran_ids, [$target_kode_cabang]);
            $stmt_check->execute($params_check);
            
            if ($stmt_check->fetchColumn() > 0) {
                $error = "Tidak dapat setor ke bank. Pastikan semua setoran dari cabang yang sama dengan rekening tujuan dan tidak ada selisih.";
            } else {
                ...
                if ($stmt_check_status->fetchColumn() > 0) {
                    ...
                    try {
                        ...
                        $stmt_rekening->execute([$rekening_cabang_id]);
                        ...
                        $sql_update_trans = "UPDATE kasir_transactions kt
                                            ...
                                            SET kt.deposit_status = 'Sudah Disetor ke Bank', kt.rekening_tujuan_id = ?
                                            WHERE sk.id IN (" . implode(',', $placeholders) . ")";
                        $stmt_update_trans = $pdo->prepare($sql_update_trans);
                        $stmt_update_trans->bindValue(1, $rekening_cabang_id, PDO::PARAM_INT);
                        foreach ($setoran_ids as $index => $id) {
                            $stmt_update_trans->bindValue($index + 2, $id, PDO::PARAM_INT);
                        }
...
if ($tab == 'terima') {
    ...
    // Add rekening filter for setor_bank - filter by cabang matching rekening
    if ($rekening_filter !== 'all' && !empty($rekening_filter)) {
        $sql_setoran .= " AND sk.kode_cabang = (SELECT kode_cabang FROM master_rekening_cabang WHERE id = ? AND status = 'active')";
        $params[] = $rekening_filter;
        error_log("Adding rekening filter with ID: " . $rekening_filter);
    }
...
// Get rekening list for dropdown with cabang relation
$sql_rekening = "
...
<div class="sidebar" id="sidebar">
    ...
    <?php if ($is_super_admin): ?>
        ...
        <a href="setoran_keuangan.php" class="active"><i class="fas fa-hand-holding-usd"></i> Manajemen Setoran</a>
...
<div class="main-content" id="mainContent">
            ...
            <div class="form-group">
                ...
                <select name="rekening_filter" class="form-control" onchange="filterByCabang(this.value)">
                    ...
                    <?php foreach ($rekening_list as $rekening): ?>
                        <option value="<?php echo $rekening['id']; ?>" <?php echo $rekening_filter == $rekening['id'] ? 'selected' : ''; ?>>
                            <?php 
                            $jenis_badge = $rekening['jenis_rekening'] == 'Mitra' ? ' [MITRA]' : ' [MILIK SENDIRI]';
                            echo htmlspecialchars($rekening['nama_cabang'] . ' - ' . $rekening['nama_bank'] . ' (' . $rekening['no_rekening'] . ')' . $jenis_badge); 
                            ?>
                        </option>
                    <?php endforeach; ?>
                </select>
        ...
        <div class="content-body">
            ...
            <form action="" method="POST" id="setorBankForm">
                <div class="form-group" style="margin-bottom: 20px;">
                    ...
                    <select name="rekening_cabang_id" id="rekeningCabang" class="form-control" required onchange="filterByCabang(this.value)">
                        <option value="">Pilih Rekening Cabang</option>
                        <?php foreach ($rekening_list as $rekening): ?>
                            <option value="<?php echo $rekening['id']; ?>" <?php echo $rekening_filter == $rekening['id'] ? 'selected' : ''; ?>>
                                <?php 
                                $jenis_badge = $rekening['jenis_rekening'] == 'Mitra' ? ' [MITRA]' : ' [MILIK SENDIRI]';
                                echo htmlspecialchars($rekening['nama_cabang'] . ' - ' . $rekening['nama_bank'] . ' (' . $rekening['no_rekening'] . ') - ' . $rekening['nama_rekening'] . $jenis_badge); 
                                ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <small style="color: var(--text-muted); font-size: 12px; margin-top: 5px;">
                        Pilih rekening sesuai cabang yang ingin disetor. Hanya setoran dari cabang yang sama yang akan ditampilkan.
                    </small>
                </div>
                ...
                <div style="margin-top: 20px;">
                    <button type="submit" name="setor_bank" class="btn btn-success" onclick="return confirm('Yakin ingin setor ke bank? Pastikan semua data sudah benar.')">
                        <i class="fas fa-university"></i> Setor ke Bank
                    </button>
    ...
    <div class="content-card">
        <div class="content-header">
            ...
            <div class="export-buttons">
                <a href="export_excel_setoran.php?type=bank_history&tab=<?php echo $tab; ?>" class="btn btn-success btn-sm">
                    <i class="fas fa-file-excel"></i> Export Excel
                </a>
                <a href="export_csv.php?type=bank_history&tab=<?php echo $tab; ?>" class="btn btn-info btn-sm">
                    <i class="fas fa-file-csv"></i> Export CSV
                </a>
                                ...
                                <?php if ($setoran_list): ?>
                                    <?php foreach ($setoran_list as $row): ?>
                                        <tr>
                                            <td><?php echo date('d/m/Y', strtotime($row['tanggal_setoran'])); ?></td>
                                            <td><code><?php echo htmlspecialchars($row['kode_setoran']); ?></code></td>
                                            <td><?php echo htmlspecialchars($row['cabang_names']); ?></td>
                                            ...
                                            <td style="text-align: center;"><?php echo $row['total_setoran_count']; ?> paket</td>
    ...
    <div class="content-card tab-validasi">
        ...
        <div class="content-body">
            <div class="workflow-info">
                ...
                <p><strong>PERBAIKAN:</strong> Validasi dilakukan per transaksi individual dengan kalkulasi yang diperbaiki untuk transaksi closing. Untuk transaksi "DARI CLOSING" yang memiliki pemasukan terkait (dipinjam), sistem akan menghitung <strong>Expected Physical Amount = Setoran Real - Jumlah yang Dipinjam</strong>. Transaksi closing ditandai dengan warna ungu dan informasi detail.</p>
            </div>
                                            ...
                                            <td class="closing-details-column">
                                                <?php if ($row['jenis_transaksi'] == 'DARI CLOSING'): ?>
                                                    <div class="expected-physical-display">
                                                        <span class="amount"><?php echo formatRupiah($row['expected_physical_amount']); ?></span>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="background: white; border-radius: 16px;">
                <div class="modal-header">
                    ...
                    <a href="?tab=validasi" class="btn-close">&times;</a>
                </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="background: white; border-radius: 16px;">
                <div class="modal-header">
                    ...
                    <a href="?tab=validasi_selisih" class="btn-close">&times;</a>
                </div>
                <form action="" method="POST">
                    <div class="modal-body">
    ...
    <?php if ($tab == 'bank_history' && isset($bank_detail_view)): ?>
    <div class="modal show">
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="background: white; border-radius: 16px;">
                <div class="modal-header">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="background: white; border-radius: 16px;">
                <div class="modal-header">
                    ...
                    <a href="?tab=bank_history&bank_detail_id=<?php echo $_GET['bank_detail_id']; ?>" class="btn-close">&times;</a>
                </div>
                <div class="modal-body">
                    <div class="closing-summary">
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($cabang_closing_detail as $detail): 
                                            if ($detail['kode_setoran'] != $current_setoran) {
                                                if ($current_setoran != '') {
                                                    echo '<tr style="background: var(--background-light); font-weight: bold;">';
                                                    echo '<td colspan="5" style="text-align: right;">Subtotal Setoran ' . $current_setoran . ':</td>';
                                                    echo '<td style="text-align: right;">√</td>';
                                                    echo '<td style="text-align: right; color: var(--success-color);">' . formatRupiah($setoran_total) . '</td>';
                                                    echo '</tr>';
                                                }
                                                $current_setoran = $detail['kode_setoran'];
                                                $setoran_total = 0;
                                            }
                                            $setoran_total += $detail['setoran_real'];
                                        ...
                                        <!-- Last subtotal -->
                                        <?php if ($current_setoran != ''): ?>
                                        <tr style="background: var(--background-light); font-weight: bold;">
                                            <td colspan="5" style="text-align: right;">Subtotal Setoran <?php echo $current_setoran; ?>:</td>
                                            <td style="text-align: right;">√</td>
                                            <td style="text-align: right; color: var(--success-color);"><?php echo formatRupiah($setoran_total); ?></td>
                                        </tr>
                                        <?php endif; ?>
                                        
                                        <!-- Grand total -->
                                        <tr style="background: var(--primary-color); color: white; font-weight: bold;">
                                            <td colspan="5" style="text-align: right;">TOTAL SETOR:</td>
                                            <td style="text-align: right;">√</td>
                                            <td style="text-align: right;"><?php echo formatRupiah(array_sum(array_column($cabang_closing_detail, 'setoran_real'))); ?></td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                ...
                <div class="modal-footer">
                    ...
                    <a href="?tab=bank_history&bank_detail_id=<?php echo $_GET['bank_detail_id']; ?>" class="btn btn-secondary">
...
function filterByCabang(rekeningId) {
    ...
    if (rekeningId === '' || rekeningId === 'all') {
        ...
        window.location.href = '?tab=setor_bank&rekening_filter=' + rekeningId;
    }
}
...
document.getElementById('setorBankForm')?.addEventListener('submit', function(e) {
    ...
    if (hasClosingTransactions) {
        const confirmMessage = `Anda akan menyetor ${checkedBoxes.length} setoran yang termasuk transaksi closing gabungan. Yakin ingin melanjutkan?`;
